...
const getAllCandidates_1 = require("@/services/qdrant/getAllCandidates");
...
exports.getAnalyticsHandler = (0, asyncHandler_1.asyncHandler)(async (_req, res) => {
    logger_1.logger.info('Fetching analytics data');
    const candidates = await (0, getAllCandidates_1.getAllCandidates)();
    ...
    const experienceDistribution = EXPERIENCE_BUCKETS.map((bucket) => ({
        range: bucket.label,
        count: candidates.filter((c) => c.experience >= bucket.min && c.experience <= bucket.max).length,
    }));
    ...
    logger_1.logger.info('Analytics computed', { totalCandidates, uniqueSkills: skills.length });
